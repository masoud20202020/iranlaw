...
The uploaded material is a collection of TXT/HTML pages.  This builder keeps the
raw files in place, uses TXT as the extraction source, groups obvious chapter/range
splits back into one document, keeps related bylaws as separate documents, removes
website chrome, extracts article boundaries when present, and writes a reproducible
seed consumed by ``load_uploaded_data.py``.
...
def iter_units() -> list[Unit]:
    units: list[Unit] = []
    categories = sorted(p for p in DATA_ROOT.iterdir() if p.is_dir() and p.name not in {"seed", "source_cache", "بهداشت_و_درمان"})
...
def main() -> None:
    ...
    content += "# Generated by scripts/build_uploaded_data_seeds.py; source files remain under data/<category>/.\n"
